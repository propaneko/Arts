...
{
    public class AOGPumpkinCropBehavior : CropBehavior
    {
        private string size = "wild";
        ...
        public AOGPumpkinCropBehavior(Block block) : base(block)
        {
        }
        ...
        private void DoSpawnVine(ICoreAPI api, BlockPos vinePos, BlockPos motherplantPos, BlockFacing facing, double currentTotalHours)
        {
            ...
            if (api.World is IServerWorldAccessor)
            {
                ...
                if (be is AOGBlockEntityPumpkinVine)
                {
                    ((AOGBlockEntityPumpkinVine)be).CreatedFromParent(motherplantPos, facing, currentTotalHours);
                }
            }
        }
