...
{
    public class DoughFormRenderer : IRenderer
    {
        ...
        public DoughFormRenderer(BlockPos pos, ICoreClientAPI capi)
        {
            this.pos = pos;
            api = capi;
        }
        ...
        private void RenderRecipeOutLine()
        {
            ...
            bool isDough = api.World.Player?.InventoryManager?.ActiveHotbarSlot?.Itemstack?.Collectible is AOCItemDough;
            if (!isDough) return;
            ...
            if (plrPos.SquareDistanceTo(pos.X, pos.Y, pos.Z) > 16) return;
        ...
        public void RegenMesh(ItemStack workitem, bool[,,] Voxels, COADoughFormingRecipe recipeToOutline, int recipeLayer)
        {
            ...
            TextureAtlasPosition tpos = api.BlockTextureAtlas.GetPosition(api.World.GetBlock(new AssetLocation("doughform")), workitem.Collectible.Code.ToShortString());
