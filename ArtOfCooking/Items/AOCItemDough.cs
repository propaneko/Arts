...
{
    public class AOCItemDough : Item
    {
        ...
        {
            return getDoughFormAnim(byEntity) ?? base.GetHeldTpHitAnimation(slot, byEntity);
        }
        public override string GetHeldTpUseAnimation(ItemSlot activeHotbarSlot, Entity forEntity)
        {
            return getDoughFormAnim(forEntity) ?? base.GetHeldTpUseAnimation(activeHotbarSlot, forEntity);
        }
        public string getDoughFormAnim(Entity byEntity)
        {
            ...
            {
                if (api.World.BlockAccessor.GetBlock(pos) is BlockDoughForm)
                {
                    return "doughform";
                }
            }
        ...
        public override void OnLoaded(ICoreAPI api)
        {
            ...
            {
                toolModes = ObjectCacheUtil.GetOrCreate(api, "doughToolModes", () =>
                {
        ...
        public override void OnHeldInteractStart(ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel, bool firstEvent, ref EnumHandHandling handling)
        {
            ...
            if (block.Attributes?.IsTrue("pieFormingSurface") == true && blockSel.Face == BlockFacing.UP && !sneaking)
            {
                ...
                Block doughformBlock = world.GetBlock(new AssetLocation("doughform"));
                if (doughformBlock == null) return;

                BlockPos belowPos = blockSel.Position.AddCopy(blockSel.Face).Down();
                Block belowBlock = world.BlockAccessor.GetBlock(belowPos);

                if (!belowBlock.CanAttachBlockAt(byEntity.World.BlockAccessor, doughformBlock, belowPos, BlockFacing.UP)) return;


                if (!world.BlockAccessor.GetBlock(placePos).IsReplacableBy(doughformBlock)) return;

                world.BlockAccessor.SetBlock(doughformBlock.BlockId, placePos);

                if (doughformBlock.Sounds?.Place?.Location != null) world.PlaySoundAt(doughformBlock.Sounds.Place.Location, blockSel.Position.X, blockSel.Position.Y, blockSel.Position.Z);

                bec = byEntity.World.BlockAccessor.GetBlockEntity(placePos) as BlockEntityDoughForm;

                if (bec != null) bec.PutDough(slot);
        ...
        {
            if (blockSel == null) return;
            if (!(byEntity.World.BlockAccessor.GetBlock(blockSel.Position) is BlockDoughForm)) return;

            BlockEntityDoughForm bea = byEntity.World.BlockAccessor.GetBlockEntity(blockSel.Position) as BlockEntityDoughForm;
            if (bea == null) return;
        ...
        public override SkillItem[] GetToolModes(ItemSlot slot, IClientPlayer forPlayer, BlockSelection blockSel)
        {
            ...
            return block is BlockDoughForm ? toolModes : null;
        }
